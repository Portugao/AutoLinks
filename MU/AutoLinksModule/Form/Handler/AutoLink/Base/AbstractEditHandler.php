...
namespace MU\AutoLinksModule\Form\Handler\AutoLink\Base;

use MU\AutoLinksModule\Form\Handler\Common\EditHandler;
use MU\AutoLinksModule\Form\Type\AutoLinkType;
 ...
 * It aims on the auto link object type.
 */
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'autoLink';
        $this->objectTypeCapital = 'AutoLink';
        $this->objectTypeLower = 'autolink';
        ...
        if ('create' == $this->templateParameters['mode']) {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->requestStack->getCurrentRequest()->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the auto link yet as other items are required which must be created before!'));
                $logArgs = ['app' => 'MUAutoLinksModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType];
    ...
    {
        return $this->formFactory->create(AutoLinkType::class, $this->entityRef, $this->getFormOptions());
    }
    ...
    protected function getFormOptions()
    {
        $options = [
            ...
            'allow_moderation_specific_creator' => $this->variableApi->get('MUAutoLinksModule', 'allowModerationSpecificCreatorFor' . $this->objectTypeCapital, false),
            'allow_moderation_specific_creation_date' => $this->variableApi->get('MUAutoLinksModule', 'allowModerationSpecificCreationDateFor' . $this->objectTypeCapital, false),
        ];
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of auto link area
        $codes[] = 'userIndex';
        // admin index page of auto link area
        $codes[] = 'adminIndex';
        // user list of auto links
        $codes[] = 'userView';
        // admin list of auto links
        $codes[] = 'adminView';
        // user list of own auto links
        $codes[] = 'userOwnView';
        // admin list of own auto links
        $codes[] = 'adminOwnView';
    ...
    protected function getDefaultReturnUrl(array $args = [])
    {
        ...
        $routePrefix = 'muautolinksmodule_' . $this->objectTypeLower . '_' . $routeArea;
    
        // redirect to the list of autoLinks
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ('create' == $this->templateParameters['mode']) {
                    $message = $this->__('Done! Auto link created.');
                } else {
                    $message = $this->__('Done! Auto link updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Auto link deleted.');
                break;
            default:
                $message = $this->__('Done! Auto link updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUAutoLinksModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'auto link', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
    ...
    protected function getRedirectUrl(array $args = [])
    {
        ...
        if ($session->has('muautolinksmodule' . $this->objectTypeCapital . 'Referer')) {
            $this->returnTo = $session->get('muautolinksmodule' . $this->objectTypeCapital . 'Referer');
            $session->remove('muautolinksmodule' . $this->objectTypeCapital . 'Referer');
        }

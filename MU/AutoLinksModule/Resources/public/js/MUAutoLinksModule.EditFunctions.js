...
function mUAutoLinksTriggerFormValidation() {
    mUAutoLinksExecuteCustomValidationConstraints(editedObjectType, editedEntityId);
...
function mUAutoLinksHandleFormSubmit(event) {
    if (triggerValidation) {
        mUAutoLinksTriggerFormValidation();
        if (!editForm.get(0).checkValidity()) {
 ...
 */
function mUAutoLinksInitEditForm(mode, entityId) {
    if (jQuery('.muautolinks-edit-form').length < 1) {
        return;
    }

    editForm = jQuery('.muautolinks-edit-form').first();
    ...
    editForm.submit(mUAutoLinksHandleFormSubmit);

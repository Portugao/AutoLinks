...
abstract class AbstractCollectionFilterHelper
{
    ...
    public function getViewQuickNavParameters($objectType = '', $context = '', array $args = [])
    {
        ...
        if ($objectType == 'autoLink') {
            return $this->getViewQuickNavParametersForAutoLink($context, $args);
        }
    ...
    {
        if ($objectType == 'autoLink') {
            return $this->addCommonViewFiltersForAutoLink($qb);
        }
    ...
    {
        if ($objectType == 'autoLink') {
            return $this->applyDefaultFiltersForAutoLink($qb, $parameters);
        }
     ...
     * @return array List of template variables to be assigned
     */
    protected function getViewQuickNavParametersForAutoLink($context = '', array $args = [])
    {
        $parameters = [];
     ...
     * @return QueryBuilder Enriched query builder instance
     */
    protected function addCommonViewFiltersForAutoLink(QueryBuilder $qb)
    {
        $request = $this->requestStack->getCurrentRequest();
        if (null === $request) {
            return $qb;
        ...
        $parameters = $this->getViewQuickNavParametersForAutoLink();
        ...
        $qb = $this->applyDefaultFiltersForAutoLink($qb, $parameters);
     ...
     * @return QueryBuilder Enriched query builder instance
     */
    protected function applyDefaultFiltersForAutoLink(QueryBuilder $qb, array $parameters = [])
    {
        $request = $this->requestStack->getCurrentRequest();
        if (null === $request) {
            return $qb;
        ...
        $routeName = $request->get('_route');
        $isAdminArea = false !== strpos($routeName, 'muautolinksmodule_autolink_admin');
        if ($isAdminArea) {
            return $qb;
        }
        ...
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved auto links only
    ...
    public function addSearchFilter($objectType, QueryBuilder $qb, $fragment = '')
    {
        ...
        if ($objectType == 'autoLink') {
            $filters[] = 'tbl.supportedString LIKE :searchSupportedString';
